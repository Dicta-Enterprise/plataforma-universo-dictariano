...
export class ProfesorManagmentService {
  ...
  ): Observable<ProfesorManagment> {
    let url = `${this.base_url}profesor`;
    return this.httpClient
      .post<IGeneric<ProfesorManagment>>(url, profesor)
      .pipe(
        map((response) => {
          return ProfesorManagment.fromJson(response.data);
        })
  ...
  editarProfesorService$(
    id: string,
    profesor: Partial<ProfesorManagment>
  ): Observable<ProfesorManagment> {
    let url = `${this.base_url}profesor/${id}`;
    return this.httpClient
      .patch<IGeneric<ProfesorManagment>>(url, profesor)
      .pipe(
        map((response) => {
          return ProfesorManagment.fromJson(response.data);
        })
